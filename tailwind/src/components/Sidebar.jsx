...
function Tooltip({ name, badge }) {
  return (
    <span
      className="
        pointer-events-none absolute left-full top-1/2 z-[9999]
        ml-3.5 -translate-y-1/2 scale-95
...
function NavItem({ item, collapsed, onNavigate }) {
  ...
  return (
    <div className="relative group">
      ...
      >
        {({ isActive }) => (
          <>
            ...
            <span
              className={`
                text-[15.5px] font-medium flex-1 whitespace-nowrap
                ...
                ${isActive ? "text-[#4338CA] !font-bold" : "text-[#4B556]"}
              `}
            >
              {item.name}
            </span>
      ...
      {collapsed && (
  <Tooltip name={item.name} badge={item.badge} />
)}
    </div>
  );
}
